...
export default function ReferredLeads() {
  ...
  const [activeTab, setActiveTab] = useState<'leads' | 'centers'>('leads');
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center bg-white p-8 rounded-3xl border border-slate-200 shadow-xl shadow-slate-200/50 gap-6">
        <div>
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-3xl font-black text-slate-900 tracking-tight">Referral Oversight</h1>
          </div>
          <p className="text-slate-500 font-medium ml-15">Audit candidacies and institutional nodes referred by Strategic BDEs.</p>
        </div>
        <div className="flex items-center gap-4 bg-slate-50 p-2 rounded-2xl border border-slate-200">
            <div className="px-5 py-2">
                <span className="text-2xl font-black text-slate-900">{data.leads.length + data.centers.length}</span>
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none">Total Sync</p>
            </div>
            <div className="w-px h-8 bg-slate-200" />
            <div className="px-5 py-2">
                <span className="text-2xl font-black text-amber-500">{data.leads.length}</span>
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none">Active Prospects</p>
            </div>
        </div>
      </div>
