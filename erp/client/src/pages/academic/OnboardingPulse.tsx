...
export default function OnboardingPulse() {
  ...
  const activeTab = (searchParams.get('tab') as 'centers' | 'students') || 'students';
  ...
  return (
    <div className="space-y-10 max-w-[1400px] mx-auto p-4 lg:p-8">
      {/* Dynamic Header */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 pb-6 border-b border-slate-100">
        <div>
          <h1 className="text-4xl font-black text-slate-900 tracking-tighter uppercase mb-2">Onboarding <span className="text-indigo-600">Traceability</span></h1>
          <p className="text-slate-500 font-medium text-sm flex items-center gap-2">
            <Activity className="w-4 h-4 text-indigo-500" />
            Institutional Audit: Verify student credentials and regional hub status.
          </p>
        </div>
        <div className="flex bg-slate-100 p-1.5 rounded-[1.5rem] border border-slate-200">
          {(['centers', 'students'] as const).map(tab => (
            <button
              key={tab}
              onClick={() => setActiveTab(tab)}
              className={`px-8 py-3 rounded-2xl text-xs font-black uppercase tracking-widest transition-all ${activeTab === tab ? 'bg-white text-slate-900 shadow-xl' : 'text-slate-500 hover:text-slate-700'}`}
            >
              {tab}
            </button>
          ))}
      ...
      {loading ? (
        <div className="h-96 flex items-center justify-center bg-white rounded-[3rem] border border-slate-100 shadow-sm">
           <div className="w-10 h-10 border-4 border-indigo-500 border-t-transparent rounded-full animate-spin" />
        </div>
      ) : (
        <div className="max-w-5xl mx-auto space-y-8">
          {/* Main Stage Timeline */}
          <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-xl shadow-indigo-900/5 relative overflow-hidden">
          ...
          <div className="space-y-6">
             <div className="flex items-center justify-between px-6">
                <h3 className="text-xl font-black text-slate-900 uppercase tracking-tighter flex items-center gap-3">
                  ...
                  {activeTab === 'students' ? 'Enrollment Status Registry' : 'Center Registration Registry'}
                </h3>
                <div className="flex items-center gap-4">
                          ...
                          cell: ({ row }: any) => (
                            <div className="flex items-center gap-3">
                              ...
                              <div className="flex flex-col">
                                ...
                                <span className="text-[10px] text-slate-400 uppercase font-black">Center: {row.original.center?.name || 'Institutional Desk'}</span>
                              </div>
                            </div>
                          )
                        },
