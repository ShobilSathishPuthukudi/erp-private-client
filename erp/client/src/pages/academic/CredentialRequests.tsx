  ...
  const fetchData = async () => {
    try {
      ...
      const [reqRes, centerRes] = await Promise.all([
        ...
        api.get('/academic/universities') // Repurposing university fetch to get centers if needed, or stick to centers
      ]);
      setRequests(reqRes.data);
      // For this demo, I'll filter departments from the university list if they are centers
      setCenters(centerRes.data.filter((d: any) => d.type === 'center' || d.type === 'university'));
    } catch (error) {
  ...
  return (
    <div className="space-y-6 max-w-[1600px] mx-auto p-4 lg:p-8">
      ...
      <Modal
        ...
        title="Initialize Credential Reveal Request"
      >
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-5 p-2">
            <div className="p-4 bg-amber-50 border border-amber-100 rounded-2xl flex items-start gap-4 mb-2">
                ...
                <div className="space-y-1">
                    <p className="text-xs font-black text-amber-900 uppercase tracking-tight">Security Protocol GAP-5</p>
                    <p className="text-sm text-amber-800 font-medium leading-snug tracking-tight">This request will be routed to Finance for risk appraisal. If approved, you will have a limited 4-hour window to reveal credentials. Your IP and access timestamp will be permanently logged.</p>
                </div>
            </div>
          ...
          <div className="space-y-4">
            <div>
              ...
              >
                <option value="">-- Select Center / University --</option>
                {centers.map(c => (
      ...
      >
        <div className="space-y-6 p-4 text-center">
            ...
            <div className="space-y-4 max-w-xs mx-auto">
                ...
                <p className="text-[10px] text-slate-400 font-bold uppercase tracking-tight">Window expires in 4 hours. Access has been logged to Global Audit Ledger.</p>
            </div>
