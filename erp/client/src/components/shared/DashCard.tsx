...
export const DashCard: React.FC<DashCardProps> = ({ 
  ...
  return (
    <button 
      onClick={onClick}
      className={`bg-slate-50/50 rounded-[2rem] border-2 border-dashed border-slate-200 p-8 flex flex-col items-center justify-center text-center group hover:bg-white hover:border-indigo-400 hover:shadow-2xl hover:shadow-indigo-500/10 transition-all duration-500 ${className}`}
    >
      ...
      <p className="text-xs text-slate-500 font-medium max-w-[220px] leading-relaxed mb-6">{description}</p>
