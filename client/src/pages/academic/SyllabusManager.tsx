...
export default function SyllabusManager() {
  ...
  const handleAddSubject = async () => {
    if (!isSubjectValid) {
      toast.error('Provide a subject name and credits greater than zero');
      return;
    }
    try {
  ...
  const columns: ColumnDef<Program>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <span className="bg-[var(--theme-soft)] border border-[var(--card-border)] text-[var(--page-text)] opacity-80 text-[10px] px-2 py-1 rounded-lg font-black uppercase tracking-widest leading-none">
           {row.original.type}
        </span>
      )
    },
  ...
  return (
    ...
    >
        <div className="overflow-hidden transition-all duration-300 flex flex-col max-h-[calc(100vh-160px)]">
            ...
            <div className="flex justify-end gap-3 p-8 border-t border-slate-100 shrink-0 uppercase">
                <button 
                    onClick={() => setSelectedProgram(null)}
                ...
                <button 
                    ...
                    className="px-8 py-4 bg-slate-900 text-white rounded-2xl text-[10px] font-black tracking-widest hover:bg-indigo-600 transition-all shadow-xl shadow-slate-900/20 active:scale-[0.98]"
                >
                    Save
                </button>
            </div>
    ...
    <Modal
      ...
      >
        <div className="space-y-5 p-2">
            <div>
                <label className="block text-xs font-black text-slate-500 uppercase tracking-widest mb-1.5 ml-1">Subject Nomenclature <span className="text-rose-500">*</span></label>
            ...
            <div>
                ...
                <div className="relative group">
                    ...
                    <input
                        type="number"
                        required
                        value={Number.isFinite(newSubject.credits) ? newSubject.credits : ''}
                        onChange={(e) => {
            ...
            <div className="pt-6 border-t border-slate-100 flex justify-end gap-3">
                ...
                <button
                    ...
                    className="px-8 py-3 bg-slate-900 text-white rounded-xl font-black hover:bg-slate-800 transition-all active:scale-95 shadow-lg shadow-slate-900/20 disabled:bg-slate-300 disabled:cursor-not-allowed disabled:shadow-none"
                >
                    Add Subject
                </button>
            </div>
