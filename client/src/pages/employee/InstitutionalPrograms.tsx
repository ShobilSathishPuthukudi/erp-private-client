  ...
  return (
    <div className="p-2 space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <PageHeader 
        ...
        description="Authorized list of all defined academic programs and vocational streams."
        icon={GraduationCap}
        action={
              ...
              <div className="flex items-center gap-2 overflow-x-auto pb-2 md:pb-0 scrollbar-hide">
                  {types.map(type => (
                      <button
                          ...
                          className={`px-6 py-3 rounded-2xl font-black text-[10px] tracking-widest border transition-all whitespace-nowrap ${
                              activeFilter === type 
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          ...
          <div key={prog.id} className="bg-white rounded-[3rem] border border-slate-200 p-10 hover:border-blue-500 transition-all group relative overflow-hidden flex flex-col justify-between">
            ...
            <div className="relative z-10">
              <div className="flex items-center gap-3 mb-4">
                  <div className="px-4 py-1.5 bg-blue-50 border border-blue-100 rounded-full">
                     <span className="text-[10px] font-black text-blue-600 tracking-widest">{prog.type}</span>
                  </div>
                 <div className={`px-4 py-1.5 border rounded-full ${
                     ...
                     <span className="text-[10px] font-black tracking-widest">{prog.status}</span>
                  </div>
              </div>
               ...
               <p className="text-sm font-bold text-slate-400 tracking-widest mb-8">
              ...
              <div className="grid grid-cols-2 gap-4 mb-10">
                 <div className="p-5 bg-slate-50 border border-slate-100 rounded-3xl">
                     <div className="flex items-center gap-2 text-slate-400 mb-2">
                        ...
                        <span className="text-[10px] font-black tracking-widest leading-none">Duration</span>
                     </div>
                 ...
                 <div className="p-5 bg-slate-50 border border-slate-100 rounded-3xl">
                     <div className="flex items-center gap-2 text-slate-400 mb-2">
                        ...
                        <span className="text-[10px] font-black tracking-widest leading-none">Credits</span>
                     </div>
            ...
            <div className="mt-auto space-y-4 pt-6 border-t border-slate-100">
               <div className="flex items-center justify-between">
                  <div>
                     <p className="text-[10px] font-black text-slate-400 tracking-widest mb-1 leading-none">Investment</p>
                     <p className="text-xl font-black text-slate-900 tracking-tighter leading-none">₹{prog.totalFee?.toLocaleString()}</p>
                  </div>
                  <div className="text-right">
                     <p className="text-[10px] font-black text-slate-400 tracking-widest mb-1 leading-none">Batch size</p>
               ...
               <div className="flex items-center gap-3 bg-slate-50 px-5 py-3 rounded-2xl border border-slate-100">
                  ...
                  <span className="text-[10px] font-bold tracking-widest text-slate-500">Sub-dept ID: {prog.subDeptId}</span>
                  <div className="ml-auto flex items-center gap-1 text-slate-300">
                    <span className="text-[10px] font-bold tracking-widest italic">ID: {prog.id}</span>
                  </div>
               </div>
            </div>
          </div>
        ...
        {filteredPrograms.length === 0 && (
          <div className="col-span-full py-24 text-center bg-white rounded-[4rem] border-2 border-dashed border-slate-100">
            ...
            <h3 className="text-2xl font-black text-slate-900 italic">Catalog node empty</h3>
            <p className="text-slate-400 font-bold text-sm tracking-widest mt-2 max-w-sm mx-auto">No programs found for institutional search criteria of your sector.</p>
          </div>
        )}
      </div>
    </div>
